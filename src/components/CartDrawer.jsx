import React from 'react';
import { X, ShoppingBag, Trash2 } from 'lucide-react';

const CartDrawer = ({ isOpen, onClose, cart, currency, exchangeRate, onRemove }) => {
  if (!isOpen) return null;

  const total = cart.reduce((sum, item) => sum + (item.price * item.quantity), 0);
  const displayTotal = currency === 'PHP' ? total * exchangeRate : total;

  return (
    <div className="fixed inset-0 z-[200] bg-white animate-in slide-in-from-right duration-300">
      {/* HEADER with X Button */}
      <div className="flex items-center justify-between px-6 py-6 border-b border-gray-100">
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">
            {cart.length} ITEMS
          </span>
        </div>
        
        {/* THE EXIT BUTTON */}
        <button 
          onClick={onClose}
          className="p-2 -mr-2 hover:rotate-90 transition-transform duration-300"
        >
          <X size={24} strokeWidth={1.5} />
        </button>
      </div>

      {/* CART CONTENT */}
      <div className="flex flex-col h-[calc(100vh-250px)] overflow-y-auto px-6 py-8">
        {cart.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full opacity-20">
            <ShoppingBag size={64} strokeWidth={1} />
            <p className="mt-4 text-[10px] font-bold uppercase tracking-[0.3em]">Cart is Empty</p>
          </div>
        ) : (
          <div className="space-y-8">
            {cart.map((item) => (
              <div key={item.id} className="flex gap-4">
                <img src={item.image} alt={item.name} className="w-20 h-20 object-cover rounded-sm" />
                <div className="flex-1">
                  <h4 className="text-xs font-bold uppercase tracking-widest">{item.name}</h4>
                  <p className="text-[10px] text-gray-400 mt-1">QTY: {item.quantity}</p>
                  <p className="text-xs font-bold mt-2">
                    {currency === 'PHP' ? '₱' : '$'}
                    {(currency === 'PHP' ? item.price * exchangeRate : item.price).toLocaleString()}
                  </p>
                </div>
                <button onClick={() => onRemove(item.id)} className="text-gray-300 hover:text-black transition-colors">
                  <Trash2 size={16} />
                </button>
              </div>
            ))}
          </div>
        )}
      </div>

      {/* FOOTER */}
      <div className="absolute bottom-0 left-0 w-full bg-white border-t border-gray-100 p-8">
        <div className="flex justify-between items-end mb-8">
          <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Total</span>
          <span className="text-2xl font-light">
            {currency === 'PHP' ? '₱' : '$'}
            {displayTotal.toLocaleString()}
          </span>
        </div>
        
        <button className="w-full bg-black text-white py-6 text-[10px] font-bold uppercase tracking-[0.3em] hover:bg-gray-900 transition-colors">
          Checkout Now
        </button>
        
        <p className="text-[8px] text-gray-300 text-center mt-6 uppercase tracking-widest">
          Free shipping on orders over {currency === 'PHP' ? '₱28,000' : '$500'}
        </p>
      </div>
    </div>
  );
};

export default CartDrawer;