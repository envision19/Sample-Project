      ...
      <nav className="flex items-center justify-between px-4 md:px-8 py-4 max-w-7xl mx-auto w-full h-16 md:h-20 gap-2 md:gap-4">
        ...
        <div className="flex items-center gap-3 md:gap-5 shrink-0">
          ...
          <button onClick={onOpenWishlist} className="relative p-1 group">
            ...
            />
            {wishlistCount > 0 && (
              <span className="absolute -top-1 -right-2 bg-black text-white text-[9px] font-black w-4 h-4 rounded-full flex items-center justify-center border-2 border-white z-20 shadow-sm">
                {wishlistCount}
              </span>
            )}
          </button>
          ...
          <button onClick={onOpenCart} className="relative p-1 group">
            <ShoppingBag size={22} strokeWidth={2.5} className="group-hover:text-gray-600 transition-colors" />
            {cartCount > 0 && (
              <span className="absolute -top-1 -right-2 bg-black text-white text-[9px] font-black w-4 h-4 rounded-full flex items-center justify-center border-2 border-white z-20 shadow-sm">
                {cartCount}
              </span>
